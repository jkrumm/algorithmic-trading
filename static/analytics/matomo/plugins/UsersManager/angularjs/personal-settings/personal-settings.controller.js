    ...
    function PersonalSettingsController(piwikApi, $filter, $window, piwik) {
        ...
        this.signupForNewsletter = function () {
            ...
            piwikApi.fetch({module: 'API', method: 'UsersManager.newsletterSignup'}).then(function () {
                ...
                var UI = require('piwik/UI');
                var notification = new UI.Notification();
                notification.show(translate('UsersManager_NewsletterSignupSuccessMessage'), {
                    id: 'newslettersignup',
                    context: 'success'
                });
                notification.scrollToNotification();
                ...
                var UI = require('piwik/UI');
                var notification = new UI.Notification();
                notification.show(translate('UsersManager_NewsletterSignupFailureMessage'), {
                    id: 'newslettersignup',
                    context: 'error'
                });
                notification.scrollToNotification();
        ...
        this.save = function () {
            ...
            if (this.doesRequirePasswordConfirmation && !this.passwordCurrent) {
                angular.element('#confirmChangesWithPassword').modal({
